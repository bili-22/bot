export default async function ({ Core, text, bot, flag: GroupFlag, alias, group, groupName, command, quote, botPermLevel }) {
    ...
    switch (params[1]) {
        ...
        case "say": {
            ...
            try {
                await bot.sendGroupMessage(params[2], text.replace(/^.*?\s+.*?\s+.*?\s+/, ""))
            } catch (error) {
        ...
        case "init": {
            ...
            if (params[3]) {
                ...
                if (group) {
                    ...
                    const groupList = [].concat(...await Promise.all(Core.bots.map(bot => bot.getGroupList().then(result => result.data))))
                    let group = groupList.find(group => group.id === +params[2])
                    if (group) {
                        targetGroup = group.id
                        groupName = group.name
            ...
            GroupFlag[targetGroup] = Core.Template[template]
            GroupFlag = GroupFlag
            await quote(`已应用模板[${template}]到[${groupName}]`)
            return
        }
        case "test": {
            console.log(await bot.getUserProfile({ qq: 1 }))
            return
        }
        default:
            await quote(help)
