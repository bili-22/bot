...
export default async function ({ Core, bot, alias, group, command, text, quote, botPermLevel }) {
    ...
    if (!params[1] || params[1] in Core.bots) {
        ...
        await quote(
            ...
            + `\n群数:${groupList.length}(${groupList.filter(group => group.id in Core.GroupFlag && Core.GroupFlag[group.id].length).length})    好友数${friendList.length}`
        )
        return
    }
    if (botPermLevel < 4) {
    ...
    switch (params[1]) {
        ...
        case "list": {
            await quote(`bot列表:\n` + Object.keys(Core.accounts).map(qq => {
                return `${Core.accounts[qq].nickname}(${qq})${qq in Core.bots ? "" : "[离线]"}`
            }).join("\n"))
            return
        }
        case "invitelist": {
